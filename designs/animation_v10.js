var animate = {
  ...
  Circle: function(i) {
    ...
    this.color = animate.color.ink[Math.floor(Math.random()*animate.color.ink.length)] + "60";
  ...
  Change: function(C) {
    for (var i = 0; i < animate.density; i = i + 0.5) { // 0.5 | 1
      ...
      var x = Math.sin(a) * (C.r - C.val * Math.tan(i / 2));
      var y = Math.tan(a) * (C.r - C.val * Math.tan(i / 1));
